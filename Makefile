...
MAKEROM_ARGS += -elf "$(TARGET).elf" -rsf "$(RSF_PATH)" -banner "$(BUILD)/banner.bnr" -icon "$(BUILD)/$(TARGET).smdh"
...
cxi:
	@$(MAKEROM) -o $(BUILD)/$(TARGET).cxi $(MAKEROM_ARGS)
	@echo "built ... $(TARGET).cxi (Code)"

cfa:
	@$(MAKEROM) -o $(BUILD)/$(TARGET).cfa -rsf $(RSF_PATH) -target t
	@echo "built ... $(TARGET).cfa (RomFS)"

cia: clean-cia cfa cxi 
	@$(MAKEROM) -f cia -o $(TARGET).cia -target t -i $(BUILD)/$(TARGET).cxi:0:0 -i $(BUILD)/$(TARGET).cfa:1:1
...
clean-exe: clean-cia
ifneq ($(WORKFLOW),1)
	@rm -f $(TARGET).3dsx $(BUILD)/$(TARGET).smdh $(TARGET).elf $(TARGET).lst $(BUILD)/banner.bnr
endif

clean-cia:
	@rm -f $(TARGET).cia $(BUILD)/$(TARGET).cxi $(BUILD)/$(TARGET).cfa
